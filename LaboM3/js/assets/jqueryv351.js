	...
	Différé: function (func) {
		var tuples = [
			...
			promesse = {
				...
				// Gardez le tube pour la compatibilité arrière
...
Data.prototype = {
	...
	access: function (propriétaire, clé, valeur) {
		...
		// quelle valeur renvoyer, respectivement:
		//
		// 1. L'ensemble de l'objet cache
		// 2. Les données stockées sur la clé
		//
		if (clé === undefined ||
				((clé && typeof clé === "chaîne") && valeur === undefined)) {
	...
	remove: function (propriétaire, clé) {
		...
		if (clé! == undefined) {
			...
			// Supporte un tableau ou une chaîne de clés séparées par des espaces
		...
		// Supprime l'expansion s'il n'y a plus de données
		if (clé === undefined || jQuery.isEmptyObject (cache)) {

			// Prise en charge: Chrome <= 35 - 45
			// Les performances de Webkit & Blink souffrent lors de la suppression des propriétés
			// à partir des nœuds DOM, donc défini sur indéfini à la place
	...
	hasData: function (propriétaire) {
		...
		retourne le cache! == non défini &&! jQuery.isEmptyObject (cache);
	}
};
...
// 2. Améliorez la maintenabilité du module en réduisant le stockage
